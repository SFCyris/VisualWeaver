...
RENDERABLE_FENCES = frozenset({
    ...
    "solve", "stats", "table", "timeline", "truth",
    "plotly", "ode", "scene", "sequence", "phylo", "reaction", "circuit",   # RICH_LANES
...
Routing — these subjects have a dedicated fence, so NEVER draw them as a mermaid/dot graph, a ```chart or an SVG: an electrical circuit, schematic or wiring → ```circuit; objects in 3-D (a part, an assembly, a room, a solid) → ```scene; a phase portrait or direction field of a dynamical system → ```ode; a histogram, box/violin plot, heatmap, sankey, treemap or sunburst → ```plotly; a DNA/RNA/protein sequence or alignment → ```sequence; a phylogenetic tree → ```phylo; a chemical reaction → ```reaction; a fractal → ```fractal. A mermaid graph is for flows, relationships and processes, not for physical things.
...
- ```mermaid — flowchart, sequence, class, state or ER diagram, and also mindmap, quadrantChart, xychart-beta, sankey-beta, block-beta and kanban (for Gantt prefer the dedicated ```gantt fence; a circuit is ```circuit, never a graph of its parts). Body: mermaid syntax, e.g. `graph TD` then `A[Start] --> B{Choice}`, or `mindmap` then an indented outline.
- ```dot — a graph best drawn by automatic layout (dependencies, call graphs). Body: Graphviz DOT, e.g. `digraph G { A -> B }`.
- ```fractal — REQUIRED for every Mandelbrot/Julia set, space-filling or self-similar curve, IFS/chaos-game figure, L-system, and strange attractor. Do not hand-draw or approximate these with ```geometry (a handful of static polygons is not a fractal — it has no iteration) or ```svg (decorative art, not the actual structure) — ```fractal genuinely computes and renders the iteration client-side; nothing else in this app can. Body: JSON. PREFER A PRESET — {"type":"NAME"} on its own renders it, and the name is all you need to get right: mandelbrot, julia, fern, sierpinski, carpet, koch (Koch snowflake), dragon, plant, frond (an L-system fern), tree, hilbert, peano, moore, gosper (flowsnake), levy, arrowhead, lorenz, rossler, thomas, halvorsen, clifford, dejong. Curve presets take "order":N (peano ≤4, gosper ≤5, moore ≤6, hilbert/koch/plant/frond ≤7, arrowhead ≤10, tree ≤13, dragon/levy ≤14). Attractor presets take "iter":N and their own named coefficients — lorenz: sigma/rho/beta; rossler: a,b,c; clifford and dejong: a,b,c,d; thomas: b; halvorsen: a. The three-dimensional ones (lorenz, rossler, thomas, halvorsen) also take "plane":"xy"|"xz"|"yz" to choose the projection; clifford and dejong are already flat. Escape-time options: "center":[x,y], "zoom":N, "iter":16-1000, and for julia "c":[re,im] e.g. {"type":"julia","c":[-0.4,0.6]} (the reader can drag a box or click to zoom). Every type accepts "palette":"fire"|"ocean"|"mono"|"viridis". Custom IFS: {"type":"ifs","maps":[[a,b,c,d,e,f,p],…]} — affine maps, PLAIN DECIMAL NUMBERS only (0.333, not the fraction 1/3 — JSON has no division operator). Custom L-system: {"type":"lsystem","axiom":"F","rules":{"F":"F+F--F+F"},"angle":60,"depth":4} — strings use letters and + - [ ] only (F/G draw, f moves, [ ] branches; other letters are placeholders). Data only — never code.
- ```geometry — an exact geometric construction (NOT for fractals/IFS/Julia/Mandelbrot — those are ```fractal, always). Body: JSON {"boundingbox":[xmin,ymax,xmax,ymin],"axis":true,"elements":[…]}. Each element is {"type":…,"args":[…],"attrs":{…}}. Coordinates must be NUMBERS. A text element takes its content as the LAST arg: {"type":"text","args":[1.5,-1.5,"a²=9"]}. Colours use fillColor/strokeColor/strokeWidth (plain fill/stroke are also accepted). Allowed types: point, line, segment, circle (centre+radius, e.g. args [[0,0],2]), ellipse (centre+radii, e.g. args [[0,0],[3,1]]), polygon, text, angle, arc, sector (arc/sector as three points, or centre+radius+start/end angle in DEGREES, e.g. args [[0,0],2,0,90]), midpoint, arrow, vector, bisector, rect (a rectangle from two opposite corners, e.g. args [[0,-0.5],[2,0.5]]), bezier / curve (a smooth cubic Bézier through its control points, e.g. args [[-4,0],[-1,3],[1,3],[4,0]]) and spline (a curve passing through every point). Text is drawn verbatim — write the actual characters (a `_`/`^` is shown literally, not as a subscript). For a dashed stroke set dash to true, or use an SVG dashArray such as 5,5. Data only — never a function or code string (use ```plot for curves).
...
- ```plotly — a statistical or data chart that ```chart cannot draw: histogram, box, violin, heatmap, contour, sankey, treemap, sunburst, candlestick, waterfall, funnel. Body: Plotly figure JSON {"data":[{"type":"histogram","x":[2.1,3.4,2.9,4.2,3.3]}],"layout":{"title":{"text":"Response times"}}}. Give RAW values and let the browser bin them and compute quartiles — never pre-bin or pre-summarise. Up to 30 traces. Use ```chart for a plain bar/line/pie.
- ```ode — a phase portrait of a 2-D dynamical system: the direction field plus trajectories, integrated in the browser. Body: `dx = y` then `dy = -sin(x)` (expressions in x and y), optional `x = -5 .. 5` and `y = -3 .. 3` for the window, one or more `start: 1, 0` initial points (up to 12; constants such as pi are fine), and `param:` sliders exactly as in ```plot (e.g. `dx = y`, `dy = -k*x - c*y`, `param: c = 0 .. 1 (0.2)`). Optional `steps: 600`, `dt: 0.02` and `arrows: 18` (6–40) tune the integration and the field density.
- ```scene — a small 3-D scene the reader rotates (drag), zooms (wheel) and pans (shift-drag): parts, assemblies, spatial setups. Body: JSON {"objects":[{"type":"box","size":[2,0.2,1],"position":[0,0.1,0],"color":"#4f8fe6"},{"type":"cylinder","radius":0.3,"height":1.2,"position":[0,0.8,0],"color":"tomato"}]}. Types: box ("size":[w,h,d]), sphere/tetrahedron/octahedron/icosahedron/dodecahedron ("radius"), cylinder and cone ("radius","height"), torus ("radius","tube"), plane ("size":[w,h], lies flat). Each object takes "position":[x,y,z], "rotation":[deg,deg,deg], "scale" (a number or [x,y,z]), "color" (any CSS colour), "opacity", "wireframe"; an unknown type is an error. Up to 200 objects. Optional "camera":{"position":[x,y,z],"target":[x,y,z]}, "grid":false, "axes":true, "background". y is up. NOT for molecules (```molecule3d) or the surface of a formula (```plot3d).
- ```sequence — a DNA/RNA/protein sequence, or an alignment of several. Body: FASTA — `>name` then the residues (a bare sequence with no header also works); several records are shown as an alignment with the columns that differ marked (headerless sequences separated by a blank line count as separate records; up to 40 sequences of 5,000 residues). Residues are coloured by base or by amino-acid class and numbered. Not for a sequence *diagram* — that is ```mermaid.
- ```phylo — a phylogenetic tree. Body: one Newick string, e.g. ((Human:0.1,Chimp:0.12):0.3,(Mouse:0.4,Rat:0.38):0.2); — with branch lengths it is a phylogram with a scale bar, without them a cladogram. Up to 200 leaves.
- ```reaction — a chemical reaction scheme drawn from structures. Body: reaction SMILES `reactants>>products`, species separated by `.`, optional agents between the arrows `A>catalyst>B`, e.g. CC(=O)O.OCC>>CC(=O)OCC.O. Up to 12 species. Use ```molecule for a single structure.
- ```circuit — ANY electrical circuit, schematic or wiring the user asks to draw (an LED circuit, a voltage divider, an RC filter…) — always this fence, never a mermaid/dot graph. Body: JSON {"components":[{"type":"battery","from":[0,2],"to":[0,0],"label":"9 V"},{"type":"resistor","from":[0,0],"to":[3,0],"label":"R1 220 Ω"},{"type":"led","from":[3,0],"to":[3,2]},{"type":"wire","from":[3,2],"to":[0,2]}]}. Types: wire, resistor, capacitor, inductor, battery, source (AC), dc, diode, led, switch, lamp, fuse, ammeter, voltmeter; ground and node take "at":[x,y]. Coordinates are grid units with y growing UPWARD (put ground at the lowest y); a component is drawn along its from→to segment with the symbol at the midpoint, and "label" is written beside it. Up to 200 components.
